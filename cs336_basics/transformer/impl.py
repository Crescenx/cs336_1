...
from cs336_basics.transformer.attn import MultiHeadSelfAttention, MultiHeadSelfAttentionRope
...
class TransformerBlockRope(nn.Module):
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int, theta: float, rotary_dim: int, device=None, dtype=None):
        super().__init__()

        self.attn = MultiHeadSelfAttentionRope(d_model, num_heads, rotary_dim, max_seq_len, theta, device=device, dtype=dtype)
        self.ln1 = RMSNorm(d_model, device=device, dtype=dtype)
        self.ffn = SwiGLU(d_model, d_ff, device=device, dtype=dtype)
        self.ln2 = RMSNorm(d_model, device=device, dtype=dtype)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (... seq_len d_model)
        attn_out = self.attn(self.ln1(x), causal=True)
        x = attn_out + x
        ff_out = self.ffn(self.ln2(x))
        x = ff_out + x
        return x
...
class TransformerLMRope(nn.Module):
    def __init__(
        self,
        vocab_size: int,
        context_length: int,
        ...
        self.layers = nn.ModuleList(
            [
                TransformerBlockRope(d_model, num_heads, d_ff, context_length, rope_theta, rotary_dim, device=device, dtype=dtype)
                for _ in range(num_layers)
            ]
        )
        self.ln_final = RMSNorm(d_model, device=device, dtype=dtype)
